...
<body>
	<div id="pageAll">
		<div class="pageTop">
			<div class="page">
                <img src="{{URL::asset('')}}public/style/img/coin02.png" /><span><a href="{{url('Index/right')}}">首页</a>&nbsp;-&nbsp;<a href="{{url('Firm/liebiao')}}">企业管理</a>&nbsp;-</span>&nbsp;企业列表
			</div>
		</div>
				...
				<div class="conShow">
					<table border="1" cellspacing="0" cellpadding="0">
						...
						<tr height="40px">
							...
							<td>
							<a href="{{url('Firm/show')}}?m_id=<?php echo $val->m_id?>" class="aColor">职位列表</a>&nbsp;&nbsp;&nbsp;&nbsp;
							<a href="{{url('Firm/firm')}}?m_id=<?php echo $val->m_id?>" class="aColor">查看</a>&nbsp;&nbsp;&nbsp;&nbsp;
...
//        alert(f_id);
        ...
        //打开
        ajax.open('get',"{{url('Firm/rz')}}?f_id="+f_id);
        //发送
        ajax.send(null);
        //设置回调函数，接收返回值
        ajax.onreadystatechange=function(){
            if(ajax.readyState==4&&ajax.status==200){
                if(ajax.responseText==0){
                    ...
                    location.href="{{url('Firm/liebiao')}}";
                }
            }
        }
    }
...
//        alert(f_id);
        ...
        var ajax=new XMLHttpRequest();
        //打开
        ajax.open('get',"{{url('Firm/fj')}}?f_id="+f_id);
        //发送
        ajax.send(null);
        //设置回调函数，接收返回值
        ajax.onreadystatechange=function(){
...
//    alert(firm_name);
    var ajax=new XMLHttpRequest();
    //打开
    ajax.open('get',"{{url('Firm/search')}}?firm_name="+firm_name);
    //发送
    ajax.send(null);
    //设置回调函数，接收返回值
    ajax.onreadystatechange=function(){
...
//        alert(wsh);
        var ajax=new XMLHttpRequest();
        //打开
        ajax.open('get',"{{url('Firm/wsh')}}?wsh="+wsh);
        //发送
        ajax.send(null);
        //设置回调函数，接收返回值
        ajax.onreadystatechange=function(){
    ...
    function checkyrz(){
        ...
        var ajax=new XMLHttpRequest();
        //打开
        ajax.open('get',"{{url('Firm/yrz')}}?yrz="+yrz);
        //发送
        ajax.send(null);
        //设置回调函数，接收返回值
        ajax.onreadystatechange=function(){
    ...
    function checkwtg(){
        ...
        var ajax=new XMLHttpRequest();
        //打开
        ajax.open('get',"{{url('Firm/wtg')}}?wtg="+wtg);
        //发送
        ajax.send(null);
        //设置回调函数，接收返回值
        ajax.onreadystatechange=function(){
